...
class SeekerDashboardPage extends StatelessWidget {
  const SeekerDashboardPage({Key? key}) : super(key: key);

  Stream<QuerySnapshot> getJobRequests() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Seeker Dashboard")),
      body: StreamBuilder<QuerySnapshot>(
        stream: getJobRequests(),
        builder: (context, snapshot) {
          if (snapshot.hasError) return Center(child: Text("Error: ${snapshot.error}"));
          if (snapshot.connectionState == ConnectionState.waiting) return Center(child: CircularProgressIndicator());

          final docs = snapshot.data!.docs;
          if (docs.isEmpty) return Center(child: Text("No job requests submitted yet."));
